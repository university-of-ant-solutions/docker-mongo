...
if [ "$originalArgOne" = 'mongod' ]; then
	...
	log "MONGO_INITDB_ROOT_USERNAME= "
	log $MONGO_INITDB_ROOT_USERNAME
	log ""
	log "MONGO_INITDB_ROOT_PASSWORD= "
	log $MONGO_INITDB_ROOT_PASSWORD
	log ""
	log "MONGO_INITDB_USERNAME= "
	log $MONGO_INITDB_USERNAME
	log ""
	log "MONGO_INITDB_PASSWORD= "
	log $MONGO_INITDB_PASSWORD
	log ""
		...
		if [ "$MONGO_INITDB_USERNAME" ] && [ "$MONGO_INITDB_PASSWORD" ]; then
			...
			"${mongo[@]}" "$rootAuthDatabase" <<-EOJS
				db.createUser({
					...
					roles: [ { role: 'dbOwner', db: $(jq --arg 'db' "$rootAuthDatabase" --null-input '$db') } ]
				})
			EOJS
